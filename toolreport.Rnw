...
characteristic_single_analysis= characteristic_single_analysis
...
firstyear = as.character(format(stardate,"%Y"))
...
We define a customized Undervaluation index using the following event characteristics: 
