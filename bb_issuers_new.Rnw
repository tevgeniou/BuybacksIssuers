...
# pandoc -s bb_issuers_new.tex --bibliography=bibliography.bib --number-sections --reference-links -o theo.docx
...
\vspace{-.2in}
\begin{abstract}
\noindent  We find that, inconsistent with the well-known low volatility anomaly, post-buyback announcement long-term abnormal returns are higher when the pre-announcement (idiosyncratic) volatility is high, and not significant otherwise. This is consistent with \cite{Stambaugh2015} who find a positive relation between stock returns and idiosyncratic volatility for undervalued stocks. The results hold for both small and large firms. We also find no post-equity issue abnormal returns when using the five-factor model proposed by Fama and French (2015a). However, the post-buyback announcement abnormal returns are also higher for firms that subsequently announce an equity issue. Finally, based on our findings we extend the Undervaluation Index of \citep{PeyerVermaelen2009}.  
\end{abstract}
\vspace{2cm}
