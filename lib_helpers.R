...
libraries_used=c("stringr","gtools","foreign","reshape2","digest","timeDate","devtools","knitr","graphics",
                 "grDevices","xtable","sqldf","stargazer",
...
car_table <- function(returns,Event.Date,Risk_Factors_Monthly,min_window = -6, max_window = 48,formula_used="(ri - RF) ~ Delta + SMB + HML + RMW + CMA") {
...
calendar_table <- function(returns,Event.Date,Risk_Factors_Monthly,min_window = -6, max_window = 48,formula_used="(ri - RF) ~ Delta + SMB + HML + RMW + CMA") {
  #data check
...
#Builds a car table like PV2009. Returns need to be monthly, same for the risk factors
car_table_old <- function(returns,Event.Date,Risk_Factors_Monthly,windows=c(1, 3, 6, 12, 24, 36, 48),formula_used="(ri - RF) ~ Delta + SMB + HML + RMW + CMA") {
  #Step 1: Build an event matrix, where all the events are aligned by event month as opposed to calendar month
  #Event.Date: if it happened on 2011-05-18, it will be set to 2011-06-01, this is our first "event" month (j=1 in PV2009)
  Event.Date <- Event.Date - as.numeric(format(Event.Date,"%d"))+1
  Event.Date <- AddMonths(Event.Date,1)
  
  #Returns: 2011-06-(01) is the monthly return value for the full month of june:
  Row.Date <- as.Date(rownames(returns))
  #Row.Date <- AddMonths(Row.Date - as.numeric(format(Row.Date,"%d"))+1,1)
  
  #Not shown here:
  # Risk_Factors_Monthly: 2011-06-(01) are the factors known at the beginning of the month of June
  Risk.Date <- as.Date(rownames(Risk_Factors_Monthly))
  
  #After this processing:
  #   01/06 returns   :   are the returns (at the end) of june
  #   01/06 factors   :   are the factors known in june, based on splitting in may
  #   01/06 eventdate : the event took place in may
  
  #Build event_month x company x factor matrix
  MAXMONTHS = max(windows)
  EVENT_ALIGNED <- array(0, c(MAXMONTHS+1, ncol(returns), ncol(Risk_Factors_Monthly)+1)) 
  for(e in 1:ncol(returns)) {
    firstHit      = which(Risk.Date==Event.Date[e])
    if(length(firstHit) > 0){
      if(firstHit + MAXMONTHS < nrow(Risk_Factors_Monthly)) {
        Factors       = Risk_Factors_Monthly[firstHit:(firstHit+MAXMONTHS),]
        firstHit      = which(Row.Date == Event.Date[e])
        upto          = min(nrow(returns),firstHit + MAXMONTHS)
        ri            = array(0,MAXMONTHS+1)
        ri[1:(upto-firstHit+1)]= returns[firstHit:upto,e] 
        Factors$ri    = ri
        EVENT_ALIGNED[,e,] <- as.matrix(Factors)
      }
    }
  }
  dimnames(EVENT_ALIGNED) <- list( c(paste("EventMonth +",c(0:48),sep="")),
                                   c(paste(colnames(returns),Event.Date)),
                                   c(colnames(Risk_Factors_Monthly),"ri"))
  
  #Step2: now for each window, calculate the CAR
  results <- array(0,c(length(windows),3))
  for (w in 1:length(windows)) {
    window = windows[w]
    #A) remove events for which no information is available in the full window
    bad <- which(AddMonths(Event.Date,window) >= max(rownames(returns)))
    if(length(bad) > 0)
      EVENT_ALIGNED_WINDOW <- EVENT_ALIGNED[,-bad,]
    else
      EVENT_ALIGNED_WINDOW <- EVENT_ALIGNED
    
    #B) The car is the sum of the monthly alpha's so calculate an LM per event month and aggregate results
    stderr <- (1:(window))*0
    alphas <- (1:(window))*0
    
    form  = as.formula(formula_used)
    for(month in 1:(window)) { 
      ret <- EVENT_ALIGNED_WINDOW[month,,]
      model = fastLm(form,data=data.frame(ret,row.names = NULL))
      alphas[month] = summary(model)$coefficients[1]
      stderr[month] = coef(summary(model))[1, "StdErr"]
    }
    #summary CAR output: aggregate results for this window
    options(warn=-1)
    tstat <- sum(alphas) / sqrt(sum(stderr*stderr)) 
    results[w,] <- c( 
      sum(alphas)*100,  #CAR in %
      tstat,            #t-statistic
      2 * pt(abs(tstat), df = df.residual(model), lower.tail = FALSE)    #pvalue
    )
    options(warn=0)
  }
  rownames(results) <- paste("+",windows,sep="")
  colnames(results) <- c("CAR","t-stat","p-value")
  return(results)
}

#Builds a calendar table like PV2009. Returns need to be monthly, same for the risk factors
calendar_table_old <- function(returns,Event.Date,Risk_Factors_Monthly,windows=c(12,24,36,48),FIVE_FACTOR=F) {
  #data check
  if (class(Risk_Factors_Monthly) != "data.frame") {
    dates <- names(Risk_Factors_Monthly)
    Risk_Factors_Monthly = data.frame(market=Risk_Factors_Monthly)
  ...
  #Set event date to start of next month
  Event.Date   <- Event.Date - as.numeric(format(Event.Date,"%d"))+1
  Event.Date   <- AddMonths(Event.Date,1)
  Return.Dates <- as.Date(rownames(returns))
  
  #which modelling formula to use  
  if(FIVE_FACTOR)
    form  = as.formula("(ri - RF) ~ Delta + SMB + HML + RMW + CMA")
  else
    form  = as.formula("(ri - RF) ~ Delta + SMB + HML")
  ...
  for(w in 1:length(windows)) {
    ...
    #returns
    ret <- returns
    for(i in 1:length(Event.Date)) {
      ret[ Return.Dates < Event.Date[i] | Return.Dates >= AddMonths(Event.Date[i],window),i] <- 0
    }
    ri <- apply(ret,1, function(x) non_zero_mean(x))
    ri <- ri[ head(which(ri !=0),1) : tail(which(ri!=0),1)]
  ...
  rownames(results) <- paste("",windows," months",sep="")
  colnames(results) <- c("Monthly Average AR","t-stat","p-value")
