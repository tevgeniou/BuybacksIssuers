...
options(warn=-1) #because some packages give warnings
...
if(!exists("input") ){#& FALSE
  load(normalizePath("input.Rdata")) #this is updated by the Rmd
  input <- newinput
}
...
SEO_exit_days = ifelse(is.null(SEO_exit_days),list(),SEO_exit_days)
BB_exit_days = ifelse(is.null(BB_exit_days),list(),BB_exit_days)
@
