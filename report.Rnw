...
if(!exists("input")){
  input = list()
  input$startdate <- as.Date("2000-12-31")
  input$enddate <- as.Date("2014-12-31")
  input$market_cap_min = 50
  input$market_cap_max = 505755
  input$quantile_used_ind = 0.2
  input$quantile_used_all = 0.4
...
source("report_tool.R")
source("report_tool_section.R")
options(warn=0)
...
latex_render_data_frame(
  ...
  label = "tbl:industriesAdditional",
  columns=NULL,
  bigtitleontop = T,
  show_rownames=TRUE,
  scale = 1,
