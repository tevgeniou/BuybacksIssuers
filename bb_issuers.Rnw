...
#dev.off();
#library(knitr); knit2pdf('bb_issuers.Rnw', texi2dvi='texi2dvi'); file.remove(paste("bb_issuers",c(".aux",".tex",".blg",".bbl",".log",".out"),sep=""))
...
The results from event studies which use either Ibbotson RATS or the Calendar Time method estimate excess returns and risks contemporaneously. Investors, such as hedge funds, who would like to exploit the anomaly and hedge market risk need however to estimate the factor betas using historical information. If the buyback signals a change in risk \citep{GrullonMichaely2004} hedging market risk may be complicated. That is why in this paper we also simulate hedged investment strategies where we estimate risk before forming the portfolios. We show that an investor who followed such a strategy starting in \Sexpr{firstyear} would by \Sexpr{lastyear} have realized cumulative excess returns between \Sexpr{round(tail(months1,1),1)}$\%$ and \Sexpr{round(tail(months48,1),1)}$\%$, depending on the length of the holding period.

The second major purpose of this paper is to test whether the buyback anomaly is related to the even better known {\it low volatility anomaly}, i.e. (idiosyncratic) volatility is negatively related to expected returns. For example, \cite{Ang2006} find {\it negative} alphas for high volatility and high idiosyncratic volatility stocks, using the 3-factor \cite{FamaFrench1993} model. Moreover, \cite{FamaFrench2015b} show that, using the 5-factor model, this anomaly persists for small firms. However, in our buyback sample we find a significant {\it positive} relation between excess returns, using the Fama-French 5-factor model as a benchmark. This is consistent with the {\it information advantage hypothesis}: in firms with high idiosyncratic risk volatility is largely driven by company-specific information, and for such firms it is more likely that management is better informed than the market. The result is also consistent with the  {\it costly arbitrage hypothesis} of \cite{Stambaugh2015} who find a positive relation between stock returns and idiosyncratic volatility for undervalued  stocks.\footnote{They  define undervaluation on the basis of a combination of 11 return anomalies reported in the literature, including net equity issuance.}  Their argument is that idiosyncratic volatility represents arbitrage risk and deters arbitrage and the resulting reduction in mispricing.  So, among underpriced stocks, the stocks with the highest idiosyncratic volatility should be the most underpriced. 

Turning to total volatility we also find that buyback authorizations of high risk stocks are followed by larger excess returns. This is consistent with the {\it option hypothesis}: a repurchase authorization is an option to take advantage of undervaluation and this option should be more valuable for high volatility stocks \citep{IkenberryVermaelen1996}. If markets underestimate the value of this option at the time of the buyback authorization, excess returns should be positively correlated with volatility. 

Combining idiosyncratic risk and total volatility with the \cite{PeyerVermaelen2009} Undervaluation Index into an Enhanced Undervaluation Index, improves the predictability of excess returns. In particular, during the four years following the buyback announcement, the high Enhanced Undervaluation Index portfolio generates an excess return of \Sexpr{round(EU_CALtable_bb["+48",13+6],2)}$\%$ per month with the Calendar Time event study method. Using the IRATS method the cumulative excess return reaches \Sexpr{round(EU_IRATStable_bb["+48",19],2)}$\%$ after 48 months. 
