...
author: "[Theodoros Evgeniou](http://faculty.insead.edu/theodoros-evgeniou/), [Enric Junque de Fortuny](http://ciri.be/), [Nick Nassuphis](https://www.linkedin.com/in/nick-nassuphis-8029372), and [Theo Vermaelen](http://faculty.insead.edu/theo-vermaelen/)"
output:  
 html_document:
 fig_width: 6
 fig_height: 2.5
...
**Note that it takes a few minutes (possibly five) to create and download the new paper - work in progress.**
...
```{r, eval=TRUE, echo = FALSE}
acceptaple_features = c("Prior.Returns.Score", "BE.ME.Score","Size.Score" ,"Valuation.Index","Industry", "Market.Cap",          "Percent.Shares", "Purpose.Code","Source.of.funds","Market.beta","SMB.beta","HML.beta" , "RMW.beta" ,"CMA.beta", "one.minus.Rsq.Score", "pre.vol.Score","ivol", "leverage.ratio")
...
feature_IRATStable_industry <- reactive({
  ...
  res = round(cbind(
    ...
    car_table_cached(CACHE,useonly_industry & DATASET_NEW()$SDC$company_subset_overvalued)$results,
    car_table_cached(CACHE,useonly_industry & DATASET_NEW()$SDC$company_subset_undervalued)$results
  ),2)[reported_times,]
...
feature_IRATStable <- reactive({
  ...
  res = round(cbind(
    ...
    car_table_cached(CACHE,Low_feature_events)$results,
    car_table_cached(CACHE,High_feature_events)$results
  ),2)[reported_times,]
  colnames(res) <- c("Low: CAR", "t-stat","p-value", "High: CAR", "t-stat","p-value")
  as.data.frame(res)
})
