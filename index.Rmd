...
author: "[Theodoros Evgeniou](http://faculty.insead.edu/theodoros-evgeniou/), [Enric Junque de Fortuny](ciri.be/), [Nick Nassuphis](https://www.linkedin.com/in/nick-nassuphis-8029372), and [Theo Vermaelen](http://faculty.insead.edu/theo-vermaelen/)"
output:  
  html_document:
    fig_width: 6
    fig_height: 2.5
...
This is an interactive tool for the article [Share Buyback and Equity Issue Anomalies Revisited](). Users can explore the effects of various parameters as well as data filtering choices. For any given choices, users can generate and download a new **customized pdf version of the paper** by clicking the button below at any time. Note that it may take a couple of minutes to create and download the new paper.
<br>
<br>
...
downloadHandler(
  ...
  content = function(file) {
    cat("knitting")
    #knitr::knit("report.Rnw",envir=environment())
    cat("texing")
    #tools::texi2pdf("report.tex")
    knitr::knit2pdf("report.Rnw",envir=globalenv())
    #toremove <- c("report.aux","report.bbl","report.blg","report.log","report.out")
    #file.remove(normalizePath(toremove))
    cat("done")
...
feature_CALtable_industry <- reactive({
  useonly_industry = !(DATASET_NEW()$SDC$Industry %in% input$industry) 
  res = round(cbind(
    calendar_table(DATASET_NEW()$returns_by_event_monthly[,useonly_industry & DATASET_NEW()$SDC$company_subset_overvalued], DATASET_NEW()$SDC$Event.Date[useonly_industry & DATASET_NEW()$SDC$company_subset_overvalued], Risk_Factors_Monthly)$results,
    calendar_table(DATASET_NEW()$returns_by_event_monthly[,useonly_industry & DATASET_NEW()$SDC$company_subset_undervalued], DATASET_NEW()$SDC$Event.Date[useonly_industry& DATASET_NEW()$SDC$company_subset_undervalued], Risk_Factors_Monthly)$results
  ),2)[reported_times,]
...
feature_CALtable <- reactive({
  ...
  Low_feature_events = which(scrub(thefeature) <= quantile(thefeature[!is.na(thefeature)],as.numeric(input$quantile_used)) & !is.na(thefeature))
  res = round(cbind(
    calendar_table(DATASET_NEW()$returns_by_event_monthly[,Low_feature_events], DATASET_NEW()$SDC$Event.Date[Low_feature_events], Risk_Factors_Monthly)$results,
    calendar_table(DATASET_NEW()$returns_by_event_monthly[,High_feature_events], DATASET_NEW()$SDC$Event.Date[High_feature_events], Risk_Factors_Monthly)$results
  ),2)[reported_times,]
  colnames(res) <- c("Low: CAL", "t-stat","p-value", "High: CAL", "t-stat","p-value")
  as.data.frame(res)
})
...
Much like the EU-index in the paper, one can create other indices by combining different firm characteristics as needed. 
...
checkboxInput("characteristic_irats1_sign", "If you would like to change the sign of this firm characteristic (Low will be High and vice versa), select here:", value = FALSE, width = "1000px")
```
...
checkboxInput("characteristic_irats2_sign", "If you would like to change the sign of this firm characteristic (Low will be High and vice versa), select here:", value = FALSE, width = "1000px")
```
...
checkboxInput("characteristic_irats3_sign", "If you would like to change the sign of this firm characteristic (Low will be High and vice versa), select here:", value = FALSE, width = "1000px")
```
...
checkboxInput("characteristic_irats4_sign", "If you would like to change the sign of this firm characteristic (Low will be High and vice versa), select here:", value = FALSE, width = "1000px")
```
...
checkboxInput("characteristic_irats5_sign", "If you would like to change the sign of this firm characteristic (Low will be High and vice versa), select here:", value = FALSE, width = "1000px")
```
...
checkboxInput("characteristic_irats6_sign", "If you would like to change the sign of this firm characteristic (Low will be High and vice versa), select here:", value = FALSE, width = "1000px")
```
